...
const Te = 100, ue = createContext(), le = createContext(), he = () => je(useContext(ue), "<A> and 'use' router primitives can be only used inside a Route."), $e = () => he().navigatorFactory(), Ke = () => he().location;
function Me(e, t = "") {
...
function Ve(e, t, r, o = {}) {
  const { signal: [n, a], utils: s = {} } = e, i = s.parsePath || ((f) => f), h = s.renderPath || ((f) => f), c = s.beforeLeave || oe(), d = D("", o.base || "");
  ...
  function ge(f, p, R) {
    untrack(() => {
      ...
      const C = !p || p[0] === "?", { replace: I, resolve: O, scroll: W, state: U } = { replace: false, resolve: !C, scroll: true, ...R }, F = O ? f.resolvePath(p) : D(C && A.pathname || "", p);
      ...
      if (F !== G || U !== v()) if (isServer) {
        ...
        J && (J.response = { status: 302, headers: new Headers({ Location: F }) }), a({ value: F, replace: I, scroll: W, state: U });
      } else c.confirm(F, R) && (L.push({ value: G, replace: I, scroll: W, state: v() }), b("navigate", { value: F, state: U }));
    });
  }
  function we(f) {
...
function it(e) {
  const t = $e(), r = Ke(), o = () => r.pathname === "/chat/login";
  return createEffect(() => {
    if (typeof document > "u" || o()) return;
    document.cookie.split(";").some((a) => a.trim().startsWith("chat_session=")) || t("/chat/login");
...
//# sourceMappingURL=app-jKQ8htjp.mjs.map
